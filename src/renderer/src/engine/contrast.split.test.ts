 ...
 * clpP vs WT cells of a single vehicle normalisation. This is omicViz's native
 * contrast shape (`condition: cell, pairs: [[clpP, WT]]`): the pooled primary rows
...
/** One veh_norm row: gene g in `cell` at `dose`, Amk-vs-H2O log2FC. */
const row = (uniqID: string, cell: string, dose: number, log2FC: number): CompareResultRow => ({
  uniqID,
  cell,
  cmpd: 'Amk',
  dose,
  time: null,
  cmp_cond: 'cmpd', // the PRIMARY axis — the contrast axis is `cell`
  comparison: 'Amk | H2O',
  mean1: null,
  mean2: null,
  sd1: null,
...
describe('contrast over a context condition (clpP vs WT from one veh_norm)', () => {
  // One veh_norm carrying both cells as context, at two doses.
  ...
  it('pairs each gene+dose across the two cells', () => {
  ...
  it('makes cell the contrast axis and keeps dose as joined context', () => {
    const r = res.rows[0]
    expect(r.cmp_cond).toBe('cell')
  ...
  it('labels the sides by the cell values, not the shared "Amk | H2O" norm label', () => {
...
describe('contrast against a dose-less vehicle (drug effect vs basal)', () => {
  // A cell comparison keeps cmpd as context: the drug sits at 2.5/5/10, the vehicle
  ...
  const rows: CompareResultRow[] = [
    row('g1', 'H2O', 0, 0.5), // basal cell difference
    row('g1', 'Amk', 2.5, 1.5),
    row('g1', 'Amk', 5, 2.5),
    row('g1', 'Amk', 10, 3.5)
  ]
  ...
  it('pairs every drug dose against the single vehicle row', () => {
    ...
    // FC2 is the basal value on every row; FC1 is that dose's cell difference.
